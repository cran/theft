...
Since we only calculated `catch22` in this vignette, the only other comparate set is `"All features"` which is automatically always included by default. This means they will both have identical results, but this functionality becomes more powerful if other feature sets were also calculated. Similar to the individual feature case, we can also use `calculate_interval` combined with `ggplot2` to summarise our findings. Here is a comparison of mean accuracy $\pm 1SD$ between feature sets:
